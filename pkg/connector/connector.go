...
func upgradeConfig(helper configupgrade.Helper) {
	...
	// Bump sidecar timeout from old default (300s/5min) to new default (900s/15min).
	// Configs generated by older versions had timeout: 300 which causes premature
	// "Request timed out" errors for long-running Claude responses.
	if val, ok := helper.Get(configupgrade.Int, "sidecar", "timeout"); ok && val == "300" {
		helper.Set(configupgrade.Int, "900", "sidecar", "timeout")
	}
}
